...
namespace testisInputDataValid
{
	...
	public:
		...
		TEST_METHOD(strLimitError)
		{
			...
			vector<ErrorInfo> errors = isInputDataValid(Text);
			vector<ErrorInfo> expectedErrors = { ErrorInfo(-1, ErrorMessage::LINES_COUNT_IS_OUT_OF_LIMIT) };
			checkAnswerVector(expectedErrors, errors);
		}
		...
		TEST_METHOD(severalErrorsInText)
		{
			...
			vector<ErrorInfo> errors = isInputDataValid(Text);
			vector<ErrorInfo> expectedErrors = { ErrorInfo(-1, ErrorMessage::LINES_COUNT_IS_OUT_OF_LIMIT), ErrorInfo(1, ErrorMessage::THE_DEFINE_DIRECTIVE_IS_NOT_ALLOWED_IN_CODE_TEXT) };
			checkAnswerVector(expectedErrors, errors);
		}
